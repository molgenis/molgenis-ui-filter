...
const providers = {
  multiFilter: async function (query:any) {
    ...
    const nameAttr = metadata.data.data.attributes.items.filter((i:any) => i.data.labelAttribute).map((i:any) => i.data.name)[0]
    // @ts-ignore
...
let id = 1
for (const filter of store.filters.available) {
